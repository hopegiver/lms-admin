export default {
    ...
    computed: {
        menusWithSub() {
            return this.menus.filter(m => m.id !== 'dashboard');
        }
    },
    mounted() {
        this.initializeFromRoute();
        window.addEventListener('hashchange', this.initializeFromRoute);
    },
    beforeUnmount() {
        window.removeEventListener('hashchange', this.initializeFromRoute);
    },
    methods: {
        initializeFromRoute() {
